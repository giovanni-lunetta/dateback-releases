    ...
    function computeStorageWarningState({
        ...
        if (showWarning) {
            ...
            warningHtml = showAbsoluteCritical
                ? '<svg width="16" height="16" viewBox="0 0 20 20" fill="none" class="storage-warning-icon"><path d="M10 2L2 17h16L10 2z" fill="var(--accent-red)" stroke="var(--accent-red)" stroke-width="1.5"/><path d="M10 8v4M10 14h.01" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg><strong>CRITICAL:</strong> You need at least 5GB free to process files safely.'
                : (isPauseAfterBatchMode
        ...
        if (!summary.isCloudModeStats) {
            if (summary.unavailableCount > 0) {
                ...
                items.push({
                    type: 'note',
                    noteClass: 'warning',
                    text: `⚠️ ${summary.unavailableCount.toLocaleString()} memories did not include local media files or download URLs in this Snapchat export.`,
                    style: null
                });
            }
            if (summary.skippedMetadataCount > 0) {
                items.push({
                    type: 'row',
                    label: 'Skipped Rows:',
                    value: summary.skippedMetadataCount.toLocaleString(),
                    valueClass: 'orange',
                    isDivider: true
                });
                items.push({
                    type: 'note',
                    noteClass: 'warning',
                    text: `⚠️ ${summary.skippedMetadataCount.toLocaleString()} memories had incomplete or invalid metadata and were skipped.`,
                    style: null
                });
            }
            items.push({
