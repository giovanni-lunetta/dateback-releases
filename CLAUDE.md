...
- **`assets/bin/mac-arm64/` contains arm64-only binaries** — `ffmpeg` and `memory-organizer` are pinned under `assets/bin/mac-arm64/`, and both production and QA `extraResources` copy from that explicit platform/architecture path. The build target must stay `arch: ["arm64"]` until x86_64 or universal binaries are added under their own platform/architecture directories.
